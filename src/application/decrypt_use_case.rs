...
pub struct DecryptUseCase<F, C, T> {
    counter: usize,
    file_service: F,
    crypto_service: C,
    terminal: T,
...
impl<F, C, T> DecryptUseCase<F, C, T>
where
    F: FileService,
    C: CryptoService,
    T: TerminalService,
{
    ...
    fn show_counter(&mut self) {
        self.counter += 1;
        self.terminal
            .print_message_in_line(format!("Дешифруется часть {}", self.counter));
    }
}

impl<F, C, T> BaseActions for DecryptUseCase<F, C, T>
where
    F: FileService,
    C: CryptoService,
    T: TerminalService,
...
mod tests {
    ...
    fn test_normal() {
        ...
        file_service.read_chunks = vec![b"encrypt_abc".to_vec(), b"encrypt_def".to_vec()];
        let decrypt_chunk = vec![b"abc".to_vec(), b"def".to_vec()];
        ...
        assert_eq!(write_chunks[0], decrypt_chunk[0]);
    }
